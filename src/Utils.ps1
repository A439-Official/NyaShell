...
function Show-SixelImage {
    ...
    if (-not (Get-Command ConvertTo-Sixel -ErrorAction SilentlyContinue)) {
        throw "未找到 ConvertTo-Sixel"
    }
    ...
    $sixelData = ConvertTo-Sixel @params
...
function Show-Image {
    ...
    if ($chafa) {
        $sizeW = if ($W -gt 0) { $W } else { [Console]::WindowWidth }
        $sizeH = if ($H -gt 0) { $H * 2 } else { [Console]::WindowHeight * 2 }
        & chafa --size "${sizeW}x${sizeH}" --format symbols --symbols block+border+space $fullPath
        return
    }

    $pythonArgs = @($fullPath)
    if ($W -gt 0) {
        $pythonArgs += @("--width", $W.ToString())
    }
    if ($H -gt 0) {
        $targetH = $H * 2
        $pythonArgs += @("--height", $targetH.ToString())
    }

    $output = & load_image @pythonArgs 2>&1
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to load image: $output"
    }
    ...
    for ($y = 0; $y -lt $height; $y += 2) {
        ...
        for ($x = 0; $x -lt $width; $x++) {
            ...
            $topColor = $colors[$topIdx]
            $botColor = $colors[$botIdx]

            $topR = [Convert]::ToInt32($topColor.Substring(1, 2), 16)
            $topG = [Convert]::ToInt32($topColor.Substring(3, 2), 16)
            $topB = [Convert]::ToInt32($topColor.Substring(5, 2), 16)

            $botR = [Convert]::ToInt32($botColor.Substring(1, 2), 16)
            $botG = [Convert]::ToInt32($botColor.Substring(3, 2), 16)
            $botB = [Convert]::ToInt32($botColor.Substring(5, 2), 16)

            $fgKey = ($topR -shl 16) -bor ($topG -shl 8) -bor $topB
            $bgKey = ($botR -shl 16) -bor ($botG -shl 8) -bor $botB

            if ($fgKey -ne $lastFg) {
                [void]$sb.Append("${esc}[38;2;${topR};${topG};${topB}m")
                $lastFg = $fgKey
            }

            if ($bgKey -ne $lastBg) {
                [void]$sb.Append("${esc}[48;2;${botR};${botG};${botB}m")
                $lastBg = $bgKey
            }
